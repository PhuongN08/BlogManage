...
{
    public class AccountListServices : IAccountListServices
    {
        private readonly BlogManageContext _context;
        private readonly IMapper _mapper;
        private readonly IPasswordHasher<Account> _hasher;

        public AccountListServices(BlogManageContext context, IMapper mapper, IPasswordHasher<Account> hasher)
        {
            _context = context;
            _mapper = mapper;
            _hasher = hasher;
        ...
        public List<AccountVM> GetAccounts(int page, int pageSize, string? search, bool? status)
        {
            var query = _context.Accounts
                ...
                .Where(a => a.Profiles.Any(p => p.Role.RoleName != "Admin"))
                .AsQueryable();
        ...
        public void UpdateAccountStatus(int accountId, bool status)
        {
            ...
            account.UpdateDate = DateOnly.FromDateTime(DateTime.Now);
        ...
        public void CreateAccountByAdmin(CreateAccountVM model)
        {
            var validRoles = new[] { "Writer", "Manager" };
            ...
            if (_context.Profiles.Any(p => p.DisplayName == model.DisplayName))
                throw new Exception("Displayname đã được sử dụng");
            ...
            try
            {
                ...
                var profile = _mapper.Map<Models.Profile>(model);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Vị trí: {ex.StackTrace}");
